...
| **Auth Feature Slice**           | `A3`            |         ✅ Done         | `Me.cs` parses `sub` using `int.TryParse` with fallback to `ClaimTypes.NameIdentifier`.                                                                                                                                     |
| **`ICurrentUser` Claim Reading** | `A4`            |         ✅ Done         | `CurrentUser.cs` checks both `JwtRegisteredClaimNames.Sub` and `ClaimTypes.NameIdentifier`.                                                                                                                                  |
| **OwnerId Assignment on Writes** | `A4`            |         ✅ Done         | `CreateCategory.cs` and `CreateTag.cs` inject `ICurrentUser` and set `OwnerId = currentUser.RequireId()`.                                                                                                                   |
| **Endpoint Authorization**       | `A5`            |         ✅ Done         | `CategoryEndpoints.cs` and `TagEndpoints.cs` map endpoints on authorization-protected route groups.                                                                                                                         |
| **Runnable HTTP Test Suite**     | `A6`            |         ✅ Done         | `TaskTracker.Api.http` updated with end-to-end multi-user registration, login, and tenant scoping isolation tests.                                                                                                          |
| **Client Implementation**        | `A7`            |         ✅ Done         | Implemented `auth.ts`, `LoginForm.tsx`, Bearer token & 401 handling in `api.ts`, and auth gating in `App.tsx`.                                                                                                            |
